...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.base_model = models.mobilenet_v3_large(models.MobileNet_V3_Large_Weights.DEFAULT)
        in_features = self.base_model.classifier[3].in_features
        self.base_model.classifier[3] = nn.Linear(in_features, 3)
...
for fold, (train_idx, test_idx) in enumerate(kf.split(df), 1):
    ...
    output_dir = f"mobile_gradcam_fold{fold}"
...
result_df.to_csv(f"kfold_predictions_with_truth_{model_name}.csv", index=False)
print(f"📁 예측 결과가 'kfold_predictions_with_truth_{model_name}.csv'에 저장되었습니다.")
